...
如果某个报告区块没有生成曲线或统计，通常说明当前脚本、日期范围或交易样本不适合用来讲这一段复盘。此时先回到脚本和样本设置，换成能产生完整报告的 `TV Backtest Skill Chart Rich Fixture v4` 或扩大日期范围，再重新打开 Strategy Tester 复查。
...
5. 如果某个报告区块没有曲线或统计，先处理策略样本或日期范围，再继续下一屏复盘。
...
| 报表空白 | 下方面板显示无数据 | 先检查脚本是否为 `strategy()`、日期范围是否有交易、样本是否足够；必要时换策略样本或扩大样本重新复查，仍无数据才记录 blocked run |
| 子图缺数据 | 某个报告块没有生成曲线或统计 | 先改成能产生该类统计的 fixture 或更长样本，再向下滚动复查 |
